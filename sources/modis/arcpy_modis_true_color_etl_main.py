...
sys.path.append("PATH ON DISK TO BASE ETL MODULES \\ReferenceNode\\ETL\\ETLScripts\\ETLBaseModules\\")
...
def executeETL(raster_catalog):
    ...
    debug_log_output_directory = os.path.join(sys.path[0], "MODIS_TRUE_COLOR_logs")
    etl_debug_logger = ETLDebugLogger(debug_log_output_directory, "MODIS_TRUE_COLOR", {                                                                                      
    ...
    etl_exception_manager = ETLExceptionManager(sys.path[0], "MODIS_TRUE_COLOR_exception_reports", {
    ...
    modis_extractor = MODISExtractor({
        ...
        "extn":image_extn,
        "subtype":[''], # ('' == MODIS True Color) list only has one item since it is the category of the raster catalog the ETL is updating
        "start_datetime":start_datetime,
        "end_datetime":end_datetime,
        'debug_logger':update_debug_log
    })
    ...
    etl_controller = ETLController(sys.path[0], "MODIS_TRUE_COLOR", {
...
def main(*args, **kwargs):
    ...
    raster_catalog = createRasterCatalog(modis_gdb.fullpath, "MODIS_TRUE_COLOR")
    ...
    if successful_new_run:
        ...
        modis_services = ("Himalaya/BHUTAN_TRUE_COLOR_AQUA", "Himalaya/BHUTAN_TRUE_COLOR_TERRA", "Himalaya/NEPAL_TRUE_COLOR_AQUA", "Himalaya/NEPAL_TRUE_COLOR_TERRA")
        modis_service = AGServiceManager(modis_services, "PATH ON DISK TO SOM TO BE RESTARTED \\ReferenceNode\\ETL\\ETLTools\\AGSSOM.exe", "localhost")
